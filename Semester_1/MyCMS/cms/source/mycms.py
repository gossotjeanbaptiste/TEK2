...
class ResponsiveDesign:
    """Gestionnaire du design responsive basé sur la résolution d'écran"""
    ...
    def __init__(self, root):
        ...
        self.determine_screen_category()

    def determine_screen_category(self):
        """Déterminer la catégorie d'écran (petit, moyen, grand)"""
    ...
    def get_window_size(self):
        """Obtenir la taille initiale de la fenêtre en fonction de l'écran"""
        width = int(self.screen_width * 0.85)
        height = int(self.screen_height * 0.90)
        return width, height

    def get_min_window_size(self):
        """Obtenir la taille minimale de la fenêtre"""
    ...
    def get_font_size(self, base_size):
        """Ajuster la taille de la police en fonction de l'écran"""
        return int(base_size * self.scale)

    def get_padding(self, base_padding):
        """Ajuster le padding en fonction de l'écran"""
        return int(base_padding * self.scale)

    def get_button_height(self):
        """Obtenir la hauteur des boutons en fonction de l'écran"""
        if self.category == "small":
            return 30
        elif self.category == "medium":
            return 35
    ...
    def get_button_width(self):
        """Obtenir la largeur des boutons en fonction de l'écran"""
        if self.category == "small":
            return 100
        elif self.category == "medium":
            return 120
    ...
    def get_container_padx(self):
        """Obtenir le padding horizontal des conteneurs"""
        if self.category == "small":
            return 10
        elif self.category == "medium":
            return 15
    ...
    def get_container_pady(self):
        """Obtenir le padding vertical des conteneurs"""
        if self.category == "small":
            return 10
        elif self.category == "medium":
            return 15
...
class MyCMS:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("🖍️ MyCMS - Gestionnaire de Contenu (CMS)")

        # Initialiser le design responsive AVANT de configurer la géométrie
        self.responsive = ResponsiveDesign(root)

        # Configurer la taille de la fenêtre en fonction de l'écran
    ...
    def _build_ui(self):
        ...
        self.main_frame.pack(fill=tk.BOTH, expand=True, padx=padx, pady=pady)
    ...
    def _create_git_tab(self):
        ...
        # Obtenir les valeurs responsive
        padx = self.responsive.get_container_padx()
        pady = self.responsive.get_container_pady()
        btn_height = self.responsive.get_button_height()
        btn_width = self.responsive.get_button_width()
        font_size_label = self.responsive.get_font_size(12)
        font_size_text = self.responsive.get_font_size(11)

        # Section Repository
        repo_section = ctk.CTkFrame(git_frame)
        repo_section.pack(fill=tk.X, padx=padx, pady=pady)

        ctk.CTkLabel(repo_section, text="Chemin du Repository:",
                     font=("Montserrat", font_size_label, "bold")).pack(anchor="w", pady=5)

        repo_frame = ctk.CTkFrame(repo_section)
        repo_frame.pack(fill=tk.X, pady=5)

        ctk.CTkEntry(repo_frame, textvariable=self.repo_path, height=btn_height).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ctk.CTkButton(repo_frame, text="📁 Parcourir",
                      command=self._browse_repo, width=btn_width, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)
        ctk.CTkButton(repo_frame, text="✓ Vérifier",
                      command=self._verify_repo, width=btn_width, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)

        # Section Actions Git
        actions_section = ctk.CTkFrame(git_frame)
        actions_section.pack(fill=tk.X, padx=padx, pady=pady)

        ctk.CTkLabel(actions_section, text="Actions Git:", font=(
            "Montserrat", font_size_label, "bold")).pack(anchor="w", pady=5)

        buttons_frame = ctk.CTkFrame(actions_section)
        buttons_frame.pack(fill=tk.X, pady=10)

        # Adapter la disposition des boutons selon l'écran
        if self.responsive.category == "small":
            button_side = tk.TOP
            button_fill = tk.X
            button_padx = 5
            button_pady = 3
        ...
        ctk.CTkButton(buttons_frame, text="⬇️  Pull (Récupérer les modifications)",
                      command=self._git_pull, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=button_side, padx=button_padx, pady=button_pady, fill=button_fill)
        ctk.CTkButton(buttons_frame, text="⬆️  Push (Envoyer les modifications)",
                      command=self._git_push, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=button_side, padx=button_padx, pady=button_pady, fill=button_fill)
        ctk.CTkButton(buttons_frame, text="📊 Statut",
                      command=self._git_status, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=button_side, padx=button_padx, pady=button_pady, fill=button_fill)
        ctk.CTkButton(buttons_frame, text="➕ Ajouter des fichiers non suivis",
                      command=self._git_add_untracked, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=button_side, padx=button_padx, pady=button_pady, fill=button_fill)

        # Message de commit
        ctk.CTkLabel(actions_section, text="Message de commit:",
                     font=("Montserrat", font_size_text)).pack(anchor="w", pady=(15, 5))
        self.commit_message = ctk.CTkEntry(
            actions_section, placeholder_text="Exemple: Mise à jour des tarifs", height=btn_height)
        ...
        status_section.pack(fill=tk.BOTH, expand=True, padx=padx, pady=pady)

        ctk.CTkLabel(status_section, text="Statut du Repository:",
                     font=("Montserrat", font_size_label, "bold")).pack(anchor="w", pady=5)

        self.git_status_display = ctk.CTkTextbox(
            status_section, height=200, state="disabled", font=("Montserrat", self.responsive.get_font_size(10)))
    ...
    def _create_editor_tab(self):
        ...
        file_section.pack(fill=tk.X, padx=padx, pady=pady)
        ...
        ctk.CTkButton(file_frame, text="🔄 Actualiser",
                      command=self._refresh_files, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)

        # Contenu éditable - Zone scrollable pour les éléments éditables
        content_section = ctk.CTkFrame(editor_frame)
        content_section.pack(fill=tk.BOTH, expand=True, padx=padx, pady=pady)
        ...
        button_frame.pack(fill=tk.X, padx=padx, pady=pady)

        ctk.CTkButton(button_frame, text="💾 Sauvegarder", command=self._save_editable_file,
                      width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)
    ...
    def _create_preview_tab(self):
        ...
        server_section.pack(fill=tk.X, padx=padx, pady=pady)
        ...
        self.server_status_label = ctk.CTkLabel(
            server_frame, text="⚫ Arrêté", font=("Montserrat", font_size_text), text_color="red")
        self.server_status_label.pack(side=tk.LEFT, padx=5)

        ctk.CTkLabel(server_frame, text="Port:", font=(
            "Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)
        self.port_entry = ctk.CTkEntry(
            server_frame, width=100, height=btn_height)
        ...
        self.server_toggle_btn = ctk.CTkButton(server_frame, text="▶️  Démarrer Serveur",
                                               command=self._toggle_server, width=btn_width, height=btn_height, font=("Montserrat", font_size_text))
        self.server_toggle_btn.pack(side=tk.LEFT, padx=5)

        # URL d'accès
        url_section = ctk.CTkFrame(preview_frame)
        url_section.pack(fill=tk.X, padx=padx, pady=pady)

        ctk.CTkLabel(url_section, text="URL de prévisualisation:",
                     font=("Montserrat", font_size_text)).pack(anchor="w", pady=5)

        url_frame = ctk.CTkFrame(url_section)
        url_frame.pack(fill=tk.X, pady=5)

        self.url_label = ctk.CTkLabel(url_frame, text="http://localhost:8000",
                                      text_color="cyan", font=("Montserrat", font_size_text, "underline"))
        self.url_label.pack(side=tk.LEFT, padx=5)

        ctk.CTkButton(url_frame, text="🌐 Ouvrir dans le navigateur",
                      command=self._open_preview, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)

        # Instructions
        info_section = ctk.CTkFrame(preview_frame)
        info_section.pack(fill=tk.BOTH, expand=True, padx=padx, pady=pady)
    ...
    5\t Cliquez sur "Arrêter Serveur" pour terminer la session
        ...
        logs_section.pack(fill=tk.BOTH, expand=True, padx=padx, pady=pady)
    ...
    def _create_settings_tab(self):
        ...
        config_section.pack(fill=tk.X, padx=padx, pady=pady)
        ...
        ctk.CTkLabel(config_section, text="Chemin du repository par défaut:", font=(
            "Montserrat", font_size_text)).pack(anchor="w", pady=(10, 5))
        self.default_repo = ctk.CTkEntry(config_section, height=btn_height)
        ...
        ctk.CTkCheckBox(config_section, text="Pull automatique au démarrage",
                        variable=self.auto_pull_var, font=("Montserrat", font_size_text)).pack(anchor="w", pady=5)
        ...
        ctk.CTkCheckBox(config_section, text="Rafraîchir automatiquement l'éditeur",
                        variable=self.auto_refresh_var, font=("Montserrat", font_size_text)).pack(anchor="w", pady=5)
        ...
        ctk.CTkButton(button_frame, text="💾 Sauvegarder les paramètres",
                      command=self._save_settings, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)
        ctk.CTkButton(button_frame, text="🔄 Réinitialiser",
                      command=self._reset_settings, width=btn_width, height=btn_height, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)
    ...
    def _create_logs_tab(self):
        ...
        ctk.CTkButton(controls_frame, text="🗑️  Effacer les logs",
                      command=self._clear_logs, width=btn_width, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)
        ctk.CTkButton(controls_frame, text="💾 Exporter les logs",
                      command=self._export_logs, width=btn_width, font=("Montserrat", font_size_text)).pack(side=tk.LEFT, padx=5)

        # Logs display
        self.logs_display = ctk.CTkTextbox(
            logs_frame, state="disabled", font=("Montserrat", self.responsive.get_font_size(10)))
        self.logs_display.pack(fill=tk.BOTH, expand=True, padx=padx, pady=pady)
...
def main():
    ...
    root = ctk.CTk()
    if getattr(sys, 'frozen', False):
        icon_path = Path(sys._MEIPASS) / 'icon' / 'logo.png'
    else:
