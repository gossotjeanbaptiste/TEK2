...
int main(void)
{
    try {
        ...
        std::cout << "=== Test OR Gate Logic ===" << std::endl << std::endl;
        
        nts::Parser parser;
        
        // Test 1: OR(1, 1) = 1
        std::cout << "Test 1: OR(1, 1) should output 1" << std::endl;
        nts::Circuit circuit1;
        parser.parseFile("tests/files/test_or.nts", circuit1);
        circuit1.setInputValue("in_x", "1");
        circuit1.setInputValue("in_y", "1");
        circuit1.simulate();
        
        auto gate1 = circuit1.getComponent("gate_or");
        ...
        if (output1 == nts::True) {
            std::cout << "  ✓ PASS: 1 OR 1 = 1" << std::endl;
        } else {
            std::cout << "  ✗ FAIL: Expected 1, got " << nts::tristateToString(output1) << std::endl;
        }
        
        // Test 2: OR(1, 0) = 1
        std::cout << std::endl << "Test 2: OR(1, 0) should output 1" << std::endl;
        nts::Circuit circuit2;
        parser.parseFile("tests/files/test_or.nts", circuit2);
        circuit2.setInputValue("in_x", "1");
        circuit2.setInputValue("in_y", "0");
        circuit2.simulate();
        
        auto gate2 = circuit2.getComponent("gate_or");
        ...
        if (output2 == nts::True) {
            std::cout << "  ✓ PASS: 1 OR 0 = 1" << std::endl;
        } else {
            std::cout << "  ✗ FAIL: Expected 1, got " << nts::tristateToString(output2) << std::endl;
        }
        
        // Test 3: OR(0, 0) = 0
        std::cout << std::endl << "Test 3: OR(0, 0) should output 0" << std::endl;
        nts::Circuit circuit3;
        parser.parseFile("tests/files/test_or.nts", circuit3);
        circuit3.setInputValue("in_x", "0");
        circuit3.setInputValue("in_y", "0");
        circuit3.simulate();
        
        auto gate3 = circuit3.getComponent("gate_or");
        ...
        if (output3 == nts::False) {
            std::cout << "  ✓ PASS: 0 OR 0 = 0" << std::endl;
        } else {
            std::cout << "  ✗ FAIL: Expected 0, got " << nts::tristateToString(output3) << std::endl;
        }
        
        // Test 4: OR(0, 1) = 1
        std::cout << std::endl << "Test 4: OR(0, 1) should output 1" << std::endl;
        nts::Circuit circuit4;
        parser.parseFile("tests/files/test_or.nts", circuit4);
        circuit4.setInputValue("in_x", "0");
        circuit4.setInputValue("in_y", "1");
        circuit4.simulate();
        
        auto gate4 = circuit4.getComponent("gate_or");
        ...
        if (output4 == nts::True) {
            std::cout << "  ✓ PASS: 0 OR 1 = 1" << std::endl;
        } else {
            std::cout << "  ✗ FAIL: Expected 1, got " << nts::tristateToString(output4) << std::endl;
        }
        
        std::cout << std::endl << "=== OR Gate Tests Complete ===" << std::endl;
        return 0;
